...
public class Production implements java.io.Serializable {
    ...
    public final ArrayList<Production> getGeneratedProductions(){
        ...
        public int Get_Number_Of_Grammar_Of_Symbols()
        {
            int number_of_grammar_of_symbols = 0;

            for(int i = 0; i < rightSide.length();)
            {
                if (!(rightSide.charAt(i)=='<'))
                {
                    number_of_grammar_of_symbols++; i++;
                }
                else
                {
                    i++;
                    while (!(rightSide.charAt(i++) =='>')) 
                    number_of_grammar_of_symbols++;
                }
            }
            return number_of_grammar_of_symbols;
        }
